...
try:
    # The stealth package may not always be available in CI environments. If
    # importing it fails we fall back to a no‑op stealth function so the
    # remainder of the code can still run.
...
def clean_html_to_markdown(html: str) -> str:
    ...
    for tag in soup(['script', 'style', 'noscript']):
        tag.decompose()
    # Remove inline-hidden elements without relying on a brittle CSS selector.
    for hidden in soup.find_all(style=True):
        style_value = (hidden.get('style') or '').replace(' ', '').lower()
...
class GrantCrawler:
    ...
    async def _apply_stealth(self, page: Page) -> None:
        ...
        If the optional playwright_stealth dependency is not available,
        this method becomes a no-op.  Having a single centralised
        location for calling stealth makes it easy to toggle this
        behaviour without changing other parts of the code.
        """
        if _stealth is not None:
            try:
                await _stealth(page)
            except Exception as exc:
                logger.debug("Stealth plugin failed: %s", exc)
        else:
            # If the stealth module isn't installed, simply return.
            return
    ...
    async def _click_with_scroll(self, page: Page, selector: str) -> None:
        """
        Scroll the element matching ``selector`` into view and click it.

        This routine is deliberately defensive because the target site keeps
        pagination elements in the DOM even when they are no longer truly
        actionable. If a regular click is intercepted, it falls back to a
        JavaScript click.
        """
    ...
    async def _extract_listings(self, page: Page) -> List[GrantSummary]:
        ...
        while True:
            ...
            # Look for a next-page button. Some pages keep the button in the
            # DOM on the last page, so we stop if clicking it no longer moves
            # to a new set of results.
            ...
            if 'disabled' in classes or 'ts-btn-disabled' in classes or aria_disabled.lower() == 'true':
                break

            previous_urls = set(results.keys())
            try:
                page_num += 1
                next_selector = 'a.ts-load-next'
                locator = page.locator(next_selector).first
                try:
                    await locator.click(force=True, timeout=5000)
                except Exception:
                    await page.eval_on_selector(next_selector, "el => el.click()")
                ...
                for _ in range(20):
                    ...
                    if current_urls and current_urls != previous_urls:
                        changed = True
                        break
                ...
                logger.info('Next-page button is no longer actionable; treating this as the last page.')
                break
