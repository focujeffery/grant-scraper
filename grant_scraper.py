...
def choose_best_candidates(
    ...
    for href, text in deduped.values():
        domain = get_domain(href)
        score = score_candidate(href, text, title, plan_source, applicable_region, query)
        if score > best_any[2]:
...
def bing_search(query: str, max_results: int = 10) -> List[Tuple[str, str]]:
    url = f"https://www.bing.com/search?q={quote_plus(query)}&setlang=zh-Hant&cc=tw"
    req = Request(
        url,
        headers={
    ...
    for a in soup.select("li.b_algo h2 a"):
        ...
        if len(results) >= max_results:
            break
    # Fallback if Bing markup changes.
    if not results:
        for a in soup.select("a[href]"):
            href = normalize_text(a.get("href") or "")
            text = normalize_text(a.get_text(" ", strip=True))
            if href.startswith("http") and get_domain(href) not in SEARCH_EXCLUDE:
                results.append((href, text))
            if len(results) >= max_results:
                break
    return results
...
class GrantCrawler:
    ...
    def _resolve_via_bing_title(self, title: str, plan_source: str, applicable_region: str) -> Tuple[str, str, str, str, str, str]:
        ...
        # Central / generic fallback queries
        cleaned_title = clean_title_for_search(title)
        generic_queries = []
        for hint in source_hints[:3]:
            generic_queries.append(f'"{title}" site:{hint}')
            generic_queries.append(f'"{cleaned_title}" site:{hint}')
            generic_queries.append(f'{cleaned_title} site:{hint}')
        ...
        seen_q = set()
        queries.extend([q for q in generic_queries if q and not (q in seen_q or seen_q.add(q))])
        # Deduplicate all queries while preserving order
        seen_all = set()
        queries = [q for q in queries if q and not (q in seen_all or seen_all.add(q))]
        ...
        for idx, q in enumerate(queries):
            ...
            organizer_url, organizer_domain, official_url, official_domain, status = choose_best_candidates(
                all_candidates, title, plan_source, applicable_region, q
            )
            # For local government rows, only accept official matches. For others,
            # stop early when we have any high-confidence official URL.
            if official_url:
                if is_local_gov:
                    status = "bing_region_official"
            ...
            # If this is not a local-government row and we have a likely best-effort
            # URL after a few searches, stop early to keep runtime reasonable.
            if (not is_local_gov) and organizer_url and idx >= 2:
                break

        organizer_url, organizer_domain, official_url, official_domain, status = choose_best_candidates(
            all_candidates, title, plan_source, applicable_region, used_query or title
        )

        if not organizer_url:
            # fallback to source/region official portal if available
            hints = source_domain_hints(plan_source, applicable_region, title)
            if hints:
                organizer_url = f"https://{hints[0]}"
                organizer_domain = hints[0]
                if is_official_domain(organizer_domain):
                    official_url = organizer_url
                    official_domain = organizer_domain
                    status = "hint_portal_fallback"
            else:
                status = "bing_title_no_match"
        else:
            if official_url:
                status = "bing_region_official" if is_local_gov else "bing_title_official"
            else:
                status = "bing_region_best_effort" if is_local_gov else "bing_title_best_effort"
    ...
    async def resolve_organizer_urls(self, row: GrantRow) -> GrantRow:
        ...
        if raw.startswith("http") and not is_google_search_url(raw) and raw_domain not in SEARCH_EXCLUDE:
            ...
            row.organizer_site_url = raw
            row.organizer_site_domain = raw_domain
            if is_official_domain(raw_domain):
                row.official_organizer_site_url = raw
                row.official_organizer_domain = raw_domain
