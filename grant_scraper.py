...
def score_candidate(url: str, title: str, plan_source: str, query: str) -> int:
...
class GrantCrawler:
    ...
    async def _accept_google_consent_if_needed(self, page: Page) -> None:
        consent_selectors = [
    ...
    async def _collect_google_candidates(self, page: Page, query: str) -> List[Tuple[str, str]]:
        url = f"https://www.google.com/search?hl=zh-TW&num=10&q={quote_plus(query)}"
        await page.goto(url, wait_until="domcontentloaded")
        await page.wait_for_timeout(1500)
        await self._accept_google_consent_if_needed(page)
        ...
        for item in items:
            ...
            if parsed.netloc.lower() in GOOGLE_HOSTS and parsed.path == "/url":
                ...
                href = normalize_text((qs.get("q") or [""])[0])
    ...
    async def _collect_bing_candidates(self, page: Page, query: str) -> List[Tuple[str, str]]:
        ...
        return [(normalize_text(h), normalize_text(t)) for h, t in [(i.get("href") or "", i.get("text") or "") for i in items] if normalize_text(h).startswith("http")]
    ...
    def _choose_best_candidates(
        ...
        for href, text in deduped.values():
            domain = get_domain(href)
            score = score_candidate(href, title=title, plan_source=plan_source, query=query)
            if score > best_any[2]:
    ...
    async def resolve_organizer_urls(
        ...
        search_queries = [f"{query} {plan_source} 官方", f"{query} {plan_source}", query]
        all_candidates: List[Tuple[str, str]] = []
        page = await context.new_page()
        try:
            await self._apply_stealth(page)
            for q in search_queries:
                logger.info("Resolving organizer URL via search: %s", q)
                try:
                    all_candidates.extend(await self._collect_google_candidates(page, q))
                except Exception as exc:
                    logger.warning("Google lookup failed for %s: %s", q, exc)
                try:
                ...
                resolved_url, resolved_domain, official_url, official_domain, status = self._choose_best_candidates(
                    all_candidates, title=title, plan_source=plan_source, query=q
                )
                if official_url:
            ...
            resolved_url, resolved_domain, official_url, official_domain, status = self._choose_best_candidates(
                all_candidates, title=title, plan_source=plan_source, query=query
            )
            if not resolved_url:
                resolved_url = raw
                resolved_domain = raw_domain
                status = "google_fallback_search_url"
            self.search_cache[cache_key] = (resolved_url, resolved_domain, official_url, official_domain, status)
            return resolved_url, resolved_domain, official_url, official_domain, status, query
        finally:
            await page.close()
