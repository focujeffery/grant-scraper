...
def source_domain_hints(plan_source: str, applicable_region: str) -> List[str]:
    hints: List[str] = []
    for key, vals in OFFICIAL_DOMAINS_BY_SOURCE.items():
        if key and key in (plan_source or ""):
            hints.extend(vals)
    for key, vals in REGION_TO_GOV_DOMAIN.items():
        if key and key in (applicable_region or ""):
            hints.extend(vals)
    deduped: List[str] = []
    for h in hints:
        if h not in deduped:
...
def score_candidate(url: str, text: str, title: str, plan_source: str, applicable_region: str, query: str) -> int:
    ...
    hints = source_domain_hints(plan_source, applicable_region)
    for hint in hints:
        if hint in domain or hint in lower_url:
            score += 80

    for token in tokenize(title)[:10]:
...
class GrantCrawler:
    ...
    def _resolve_via_bing_title(self, title: str, plan_source: str, applicable_region: str) -> Tuple[str, str, str, str, str, str]:
        ...
        source_hints = source_domain_hints(plan_source, applicable_region)
        queries = []
        for hint in source_hints[:3]:
            queries.append(f'{title} {plan_source} site:{hint}')
        queries.extend([
        ...
        if cache_key in self.search_cache:
            ...
            return organizer_url, organizer_domain, official_url, official_domain, status, queries[0]
        ...
        for q in queries:
            ...
            try:
                all_candidates.extend(bing_search(q))
            except Exception as exc:
